...
const app = express();
// IMPORTANTE: Render define a porta na variável PORT
...
// Servir arquivos estáticos (ajustar caminho para o Render)
app.use(express.static(path.join(__dirname, '../frontend')));
app.use('/admin', express.static(path.join(__dirname, 'admin')));

// ===== FUNÇÕES AUXILIARES =====
// Usar caminho absoluto para os dados
...
function inicializarDados() {
    ...
    if (produtos.length === 0) {
        escreverJSON('produtos', produtosIniciais);
    }
...
app.get('/api/teste', (req, res) => {
    res.json({ mensagem: 'Backend Pretholas funcionando! ✅', status: 'online', ambiente: process.env.RENDER ? 'Render' : 'Local' });
});
...
app.post('/api/pedidos', (req, res) => {
    ...
    if (clienteExistente) {
        clienteExistente.totalCompras = (clienteExistente.totalCompras || 0) + req.body.total;
...
// Configurações
app.use(cors());  // Permite qualquer frontend acessar
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

// Servir apenas o admin (painel)
app.use('/admin', express.static(path.join(__dirname, 'admin')));

// O frontend (loja) fica em outro servidor, não aqui!
